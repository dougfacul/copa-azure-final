...
5. **Ativar a identidade gerenciada do Web App** (System-assigned): `app-prd-tk-fend-cin-001` → **Settings → Identity → System assigned** → **Status: On** → **Save** (confirme). Isso cria uma identidade do app no Entra ID — **sem segredo** em lugar nenhum — que ele usará para ler o cert do vault.
6. **Dar ao Web App acesso de leitura no vault:** o **Key Vault** → **Access control (IAM)** → **+ Add → Add role assignment** → role **Key Vault Secrets User** → **Members → Managed identity** → selecione o Web App `app-prd-tk-fend-cin-001` → **Review + assign**. _(O cert + chave são lidos como *secret*; se quiser granularidade, some também **Key Vault Certificate User**.)_
7. **Importar o certificado no Web App a partir do Key Vault:** `app-prd-tk-fend-cin-001` → **Settings → Certificates → Bring your own certificates (.pfx) → Import from Key Vault** → selecione o vault e o certificado → **Add**. _(Agora funciona, porque a **identidade do app** (passo 5) tem leitura no vault (passo 6).)_
...
> 🔐 **Por que RBAC + Managed Identity?** É a **boa prática**: ninguém usa senha/connection string para o vault — **você** ganha um papel de data-plane para gerenciar o cert, e o **Web App** usa a **própria identidade gerenciada** (sem segredo) para ler. É o mesmo padrão que a §7 propõe para tirar segredos da config.
